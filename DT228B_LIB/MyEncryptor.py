    ...
    def sha1Encode(self, text):
        if not os.path.isfile(str(text)):  # check if ist a file
            if not text:  # check if its a string
                ...
                return (m.hexdigest())  # return the hash by calling hexdigest Method which retuns the hash value
        else:
            BLOCKSIZE = 65536  # set the block size see : http://www.dhillonblog.com/2014/07/understanding-io-request-block-size/
            hasher = hashlib.sha1()
            with open(text, 'rb') as afile:  # open file in read byte mode
                ...
                while len(buf) > 0:  # while the length of the buffer is greater thena zero keep iterating
                    hasher.update(buf)  # append buffer to the cuurent value in the hasher object
                    buf = afile.read(BLOCKSIZE)
            return (
            hasher.hexdigest())  # finally return the hash by calling hexdigest Method which retuns the hash value
